...
create or replace function livnow.dmlapi_livnow_especialidade_turno_purge(
    fv_id uuid,
    fv_usuario_id uuid,
...
------------------------------------------------------------------
-- dmlapi_livnow_especialidade_turno_purge: inactive record
------------------------------------------------------------------
declare
    lr_data    livnow.livnow_especialidade_turno;
begin
    if (fv_id is not null) then
        lr_data := livnow.dmlapi_livnow_especialidade_turno_select(fv_id      => fv_id,
                                                                fv_locking => true);
        if (lr_data.id is not null) then
            ...
            ------------------------------------
            perform livnow.dmlapi_livnow_especialidade_turno_merge(fr_data => lr_data);
            ------------------------------------
        end if;
    end if;
